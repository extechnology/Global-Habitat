    ...
    <section id="about" className="relative bg-white py-28 md:py-40">
      <div className="container-luxe grid lg:grid-cols-12 gap-14 lg:gap-20 items-center">
        <Reveal direction="mask" className="lg:col-span-6 relative">
          ...
          <div className="absolute -bottom-8 -right-4 md:-right-8 hidden md:block glass rounded-sm px-7 py-6 max-w-[260px] shadow-[var(--shadow-soft)]">
        ...
        <div className="lg:col-span-6">
          ...
          <Reveal delay={0.1}>
            <h2 className="text-display mt-5 text-[clamp(2rem,4.4vw,3.6rem)] text-[var(--ink)]">
              Architecture as a <span className="italic">quiet</span> language
              of home.
            </h2>
          </Reveal>
          <Reveal delay={0.2}>
            <p className="mt-7 text-[var(--muted-foreground)] leading-[1.85] max-w-xl">
          ...
          <Stagger className="mt-14 grid grid-cols-2 gap-y-10 gap-x-6 border-t border-[var(--border)] pt-10">
            {stats.map((s) => (
              <motion.div variants={staggerItem} key={s.label}>
                ...
                <p className="mt-3 text-xs uppercase tracking-[0.2em] text-[var(--muted-foreground)]">
                  {s.label}
                </p>
              </motion.div>
            ))}
