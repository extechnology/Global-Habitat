...
function Amenities() {
  ...
  return (
    <section
      id="amenities"
      className="relative bg-[var(--accent)] py-28 md:py-36 overflow-hidden"
    >
      <div className="container-luxe">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14">
          ...
          <Reveal direction="right">
            <p className="text-[var(--muted-foreground)] text-sm uppercase tracking-[0.25em]">
              Drag to explore →
            </p>
          </Reveal>
        </div>
        ...
        <div className="no-scrollbar flex gap-5 overflow-x-auto snap-x snap-mandatory pb-2">
          {items.map((it, i) => (
            <Reveal key={it.title} delay={i * 0.05}>
              <div className="snap-start shrink-0 w-[260px] md:w-[300px] aspect-[3/4] rounded-sm bg-white border border-[var(--border)] p-8 flex flex-col justify-between hover:bg-[var(--ink)] hover:text-white transition-colors duration-700 group">
                <span className="text-display italic text-[var(--gold)] text-lg">
                  0{(i + 1).toString().padStart(2, "0")}
                </span>
                <div>
                  <it.icon className="h-9 w-9 stroke-[1.25] text-[var(--ink)] group-hover:text-[var(--gold)] transition-colors" />
